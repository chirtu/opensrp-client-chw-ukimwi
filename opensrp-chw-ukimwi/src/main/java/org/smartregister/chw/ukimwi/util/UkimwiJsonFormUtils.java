...
import static org.smartregister.chw.ukimwi.util.Constants.ENCOUNTER_TYPE;
import static org.smartregister.chw.ukimwi.util.Constants.STEP_ONE;
import static org.smartregister.chw.ukimwi.util.Constants.STEP_TWO;

public class UkimwiJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = ukimwiFormFields(jsonForm);
    ...
    public static JSONArray ukimwiFormFields(JSONObject jsonForm) {
        try {
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.UKIMWI_CONFIRMATION.equals(encounter_type)) {
            encounter_type = Constants.TABLES.UKIMWI_CONFIRMATION;
        } else if (Constants.EVENT_TYPE.UKIMWI_FOLLOW_UP_VISIT.equals(encounter_type)) {
            encounter_type = Constants.TABLES.UKIMWI_FOLLOW_UP;
        }
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = UkimwiLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = UkimwiLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(UkimwiLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(UkimwiLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(UkimwiLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
