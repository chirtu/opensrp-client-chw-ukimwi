...
public class UkimwiUtil {

    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            UkimwiJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(UkimwiJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static ECSyncHelper getSyncHelper() {
        return UkimwiLibrary.getInstance().getEcSyncHelper();
    }

    public static ClientProcessorForJava getClientProcessorForJava() {
        return UkimwiLibrary.getInstance().getClientProcessorForJava();
    }
    ...
    public static boolean launchDialer(final Activity activity, final BaseUkimwiCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> UkimwiUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        AllSharedPreferences allSharedPreferences = UkimwiLibrary.getInstance().context().allSharedPreferences();
        Event baseEvent = UkimwiJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        UkimwiUtil.processEvent(allSharedPreferences, baseEvent);
    }
    ...
    public static class CloseUkimwiMemberFromRegister extends AsyncTask<Void, Void, Void> {
        private final String baseEntityId;

        public CloseUkimwiMemberFromRegister(String baseEntityId) {
            this.baseEntityId = baseEntityId;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            UkimwiDao.closeUkimwiMemberFromRegister(baseEntityId);
            return null;
        }
